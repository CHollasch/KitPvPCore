...
*
* You are welcome to use it, modify it and redistribute it under the following conditions: 1. Don't
* claim this class as your own 2. Don't remove this text
*
...
public final class ReflectionHandler {
	...
	*/
	...
	public static <T extends Class> List<T> getClassesInPackage(String pckage, T clazz) {
		ArrayList<T> modules = Lists.newArrayList();
		CodeSource cs = KitPvP.getPlugin_instance().getClass().getProtectionDomain().getCodeSource();
		URL resource = cs.getLocation();
		String relPath = pckage.replace('.', '/');
